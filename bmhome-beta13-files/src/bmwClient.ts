...
export class BMWClient {
  ...
  private descriptorState: Record<string, DescriptorEntry> = {};
  ...
  private handleMqttMessage(topic: string, payload: Buffer): void {
    try {
      ...
      if (json?.data && typeof json.data === 'object' && !Array.isArray(json.data)) {
        ...
        this.updateVehicleStateFromDescriptors(vin, json);
      } else {
  ...
  private updateVehicleStateFromDescriptors(vin: string, rawPayload: any): void {
    const soc = this.firstNumber([
      'vehicle.powertrain.electric.battery.stateOfCharge.displayed',
      'vehicle.powertrain.electric.battery.stateOfCharge.current',
      'vehicle.drivetrain.highVoltageBattery.stateOfCharge',
      'vehicle.drivetrain.highVoltageBattery.soc',
    ]);

    const remainingRange = this.firstNumber([
      'vehicle.drivetrain.electricEngine.kombiRemainingElectricRange',
      'vehicle.drivetrain.lastRemainingRange',
      'vehicle.powertrain.electric.range.target',
    ]);

    const chargingStatus = this.firstString([
    ...
    const lockStatus = this.normaliseLockStatus(this.firstString([
      'vehicle.security.centralLock.status',
      'vehicle.vehicle.lock.status',
      'vehicle.vehicle.locking.status',
    ]));

    const isCharging = this.normaliseChargingState(chargingStatus);

    const preconditionActive = this.normaliseActiveState(this.firstValue([
    ...
    const doorsOpen = this.anyOpenBoolean([
    ...
    const windowsOpen = this.anyWindowOpen([
    ...
    const alarmActive = this.normaliseActiveState(this.firstValue([
      'vehicle.vehicle.antiTheftAlarmSystem.alarm.isOn',
      'vehicle.security.alarm.isActive',
    ]));

    const tyrePressures = [
      this.firstNumber(['vehicle.chassis.axle.row1.wheel.left.tire.pressure']),
      this.firstNumber(['vehicle.chassis.axle.row1.wheel.right.tire.pressure']),
      this.firstNumber(['vehicle.chassis.axle.row2.wheel.left.tire.pressure']),
      this.firstNumber(['vehicle.chassis.axle.row2.wheel.right.tire.pressure']),
    ...
    console.log(
      '[BMWClient] Parsed vehicle state: ' +
        `SOC=${soc ?? 'unknown'} ` +
        `Range=${remainingRange ?? 'unknown'} ` +
        `Charging=${isCharging ?? 'unknown'} ` +
        `Lock=${lockStatus ?? 'unknown'} ` +
        `DoorsOpen=${doorsOpen ?? 'unknown'} ` +
        `WindowsOpen=${windowsOpen ?? 'unknown'} ` +
  ...
  private firstValue(paths: string[]): unknown {
    for (const descriptorPath of paths) {
      const entry = this.descriptorState[descriptorPath];
  ...
  private firstNumber(paths: string[]): number | undefined {
    const value = this.firstValue(paths);
    const numberValue = Number(value);
    return Number.isFinite(numberValue) ? numberValue : undefined;
  }

  private firstString(paths: string[]): string | undefined {
    const value = this.firstValue(paths);
    return typeof value === 'string' ? value : undefined;
  }

  private anyOpenBoolean(paths: string[]): boolean | undefined {
    let seen = false;

    for (const descriptorPath of paths) {
      const value = this.firstValue([descriptorPath]);
      if (value === undefined) {
        continue;
      }
  ...
  private anyWindowOpen(paths: string[]): boolean | undefined {
    let seen = false;
  ...
  private normaliseChargingState(value?: string): boolean | undefined {
    if (!value) {
      return undefined;
    }

    const upper = value.toUpperCase();
    if (['CHARGINGACTIVE', 'CHARGING', 'ACTIVE', 'CONNECTED'].includes(upper)) {
      return true;
    }

    if (['NOCHARGING', 'CHARGINGPAUSED', 'CHARGINGENDED', 'DISCONNECTED', 'ERROR', 'INVALID'].includes(upper)) {
      return false;
    }
  ...
  private normaliseActiveState(value: unknown): boolean | undefined {
    ...
    const upper = value.toUpperCase();
    if (['TRUE', 'ACTIVE', 'ON', 'ON_CHARGE', 'ON_DRIVE', 'RUNNING', 'PRECONDITIONING'].includes(upper)) {
      return true;
    }

    if (['FALSE', 'INACTIVE', 'OFF', 'TEMP_OFF', 'UNKNOWN', 'INVALID'].includes(upper)) {
      return false;
    }
  ...
  private applyTokenResponse(data: any): void {
    ...
    this.tokenStore.gcid = decoded?.gcid || decoded?.sub || decoded?.['https://customer.bmwgroup.com/gcid'] || this.tokenStore.gcid;
